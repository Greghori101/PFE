...
function SelectSupervisors() {
    ...
    const show = (event) => {
        ...
        if (event.target.value > supervisors.length) {
            setNum(0);
            numPage = 0;
            setPage(supervisors);
        } else {
            setPage(supervisors.slice(0, event.target.value));
            setNumbers(
                range(1, Math.ceil(supervisors.length / event.target.value))
            );
            setNum(0);
            numPage = 0;
        }
    ...
    const minus = () => {
        setPage(supervisors.slice((num - 1) * length, num * length));
        setNum(num - 1);
    };
    const plus = () => {
        setPage(supervisors.slice((num + 1) * length, (num + 2) * length));
        setNum(num + 1);
    };
    ...
    const selectPage = (event) => {
        ...
        setNum(numPage);
        setPage(supervisors.slice(numPage * length, (numPage + 1) * length));
    };
    const filter = (input) => {
        if (input.length > 0) {
            let result = [];
    ...
    const selectSupervisor = async (id) => {
        let url = urls + "/groups/select_supervisor/" + id;
        let options = {
            method: "put",
            url: url,
            headers: {
    ...
    const deleteSupervisor = async (userId) => {
        const Swal = require("sweetalert2");
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.isConfirmed) {
                let url = urls + "/groups/deselect_supervisor/" + userId;
                let options = {
                    method: "DELETE",
                    url: url,
                    headers: {
    ...
    return (
        <div className="card-body">
                    ...
                    >
                        <thead className="text-dark-tp3 bgc-grey-l4 text-90 border-b-1 brc-transparent">
                            <tr>
                                ...
                                <th width="160px">Select Team Leader</th>
                            </tr>
                        </thead>
            ...
            {page.length < supervisors.length ? (
                <div className="row">
                    <div className="col-md-6 align-self-center">
                        ...
                        >
                            ...
                            {num * length + page.length} of {supervisors.length}
                        </p>
                    </div>
